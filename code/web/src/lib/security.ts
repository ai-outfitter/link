...
export function authorizeRequest(request: Request): string | null {
  ...
  const trustedNetwork = process.env.LINK_TRUST_NETWORK === "1";
  if (!WILDCARD.has(configuredHost) && !LOOPBACK.has(host) && host !== configuredHost) return "untrusted host";
  if (nonLoopback && !trustedNetwork) {
